...
export default function Popular() {
    ...
    const chunkedpop = [];
    for (let i = 0; i < pop.length; i += cardsPerSlide) {
        chunkedpop.push(pop.slice(i, i + cardsPerSlide));
    }
    ...
    return (
        <div id="carouselExample3" className="carousel slide now-playing" data-bs-ride="carousel">
            <div className="carousel-inner">
                <div className="top d-flex justify-content-between align-items-center px-5 me-5">
                    <h1 className='text-white mx-5'>Popular Movies :</h1>
                    <Link to={`/movies`} className="link-more text-decoration-none text-white me-5 p-1 rounded bg-danger">
                        Show More <BiRightArrowAlt />
                    </Link>
                </div>
                {
                    chunkedpop.map((group, popIndex) => (
                        <div className={`carousel-item ${popIndex === 0 ? 'active' : ''}`} key={popIndex}>
