...
export default function AiringToday() {
    ...
    const chunkedAiring = [];
    for (let i = 0; i < airing.length; i += cardsPerSlide) {
        chunkedAiring.push(airing.slice(i, i + cardsPerSlide));
    }
    ...
    return (
        <div id="carouselExample7" className="carousel slide now-playing" data-bs-ride="carousel">
            <div className="carousel-inner">
                <div className="top d-flex justify-content-between align-items-center px-5 me-5">
                    <h1 className='text-white mx-5'>Airing Series Today : </h1>
                    <Link to={`/series`} className={`link-more text-decoration-none text-white p-1 rounded bg-danger`}>
                        Show More <BiRightArrowAlt />
                    </Link>
                </div>
                {
                    chunkedAiring.map((group, airingIndex) => (
                        <div className={`carousel-item ${airingIndex === 0 ? 'active' : ''}`} key={airingIndex}>
