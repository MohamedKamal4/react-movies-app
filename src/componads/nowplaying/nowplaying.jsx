...
export default function NowPlaying() {
    ...
    const chunkednow = [];
    for (let i = 0; i < now.length; i += cardsPerSlide) {
        chunkednow.push(now.slice(i, i + cardsPerSlide));
    }
    ...
    return (
        <div id="carouselExample2" className="carousel slide now-playing" data-bs-ride="carousel">
            <div className="carousel-inner">
                <div className="top d-flex justify-content-between align-items-center px-5 me-5">
                    <h1 className='text-white mx-5'>Now Playing Movies :</h1>
                    <Link to={`/movies`} className="link-more text-decoration-none text-white me-5 p-1 rounded bg-danger">
                        Show More <BiRightArrowAlt />
                    </Link>
                </div>
                {
                    chunkednow.map((group, nowIndex) => (
                        <div className={`carousel-item ${nowIndex === 0 ? 'active' : ''}`} key={nowIndex}>
