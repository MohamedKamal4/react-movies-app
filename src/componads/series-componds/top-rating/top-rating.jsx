...
export default function TopRateSeries() {
    ...
    return (
        <div id="carouselExample10" className="carousel slide now-playing" data-bs-ride="carousel">
            <div className="carousel-inner ">
                <div className="top d-flex justify-content-between align-items-center px-5 me-5">
                    <h1 className='text-white mx-5'>Top Series Rating : </h1>
                    <Link to={`/series`} className={`link-more text-decoration-none text-white p-1 rounded bg-danger`}>
                        Show More <BiRightArrowAlt />
                    </Link>
                </div>
                {
                    chunkedtop.map((topGroup, topIndex) => (
                        <div className={`carousel-item ${topIndex === 0 ? 'active' : ''}`} key={topIndex}>
                            <div className="d-flex justify-content-center gap-3 py-4">
                                {
                                    topGroup.map(item => (
                                        <div key={item.id}>
                                            <Link to={`/tv/${item.id}`} className="link-more text-decoration-none text-white">
                                                <div className="now-card card modern-card text-white" style={{ width: '15rem' }}>
                                                    <img src={`https://image.tmdb.org/t/p/w300${item.poster_path}`} className="card-img-top" alt={item.name} />
            ...
            <button className="carousel-control-prev" type="button" data-bs-target="#carouselExample10" data-bs-slide="prev">
                <span className="carousel-control-prev-icon" aria-hidden="true"></span>
                <span className="visually-hidden">Previous</span>
            </button>
            <button className="carousel-control-next" type="button" data-bs-target="#carouselExample10" data-bs-slide="next">
